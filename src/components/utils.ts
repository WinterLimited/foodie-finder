...
export const loadUserVectorFromLocalStorage = (): { price_level: number, rating: number, types: number } => {
    const userVectorString = localStorage.getItem('selectedRestaurant');
    if (userVectorString) {
        return JSON.parse(userVectorString) as { price_level: number, rating: number, types: number };
    }
    return { price_level: 0, rating: 0, types: 0 }   ; // price_level, rating은 0, types는 빈 배열을 기본값으로 반환
}

export const calculateCosineSimilarity = (userVector: { price_level: number, rating: number, types: number }, restaurantData: Restaurant[]): Restaurant[] => {
    ...
    return restaurantData.map((restaurant: Restaurant) => {
        ...
        if (userVector) {
            restaurant.cosineSimilarity = cosineSimilarity(userVectorArray, restaurantVector);
        }
        return restaurant;
    });
}
...
export const convertToVector = (user: {price_level: number, rating: number, types: string[]}): number[] => {
